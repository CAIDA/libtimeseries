...
static int server_connect(tsmq_server_t *server)
{
  ...
  if (zmq_send(server->broker_socket, &msg_type_p, 1, 0) == -1) {
    tsmq_set_err(server->tsmq, errno, "Could not send ready msg to broker");
    return -1;
  }
...
static int handle_key_lookup_bulk(tsmq_server_t *server)
{
  ...
  if (keys_cnt > server->key_alloc_cnt) {
    /* do the mallocs */
    if (((server->keys = realloc(server->keys, sizeof(char *) * keys_cnt)) ==
         NULL) ||
        ((server->key_ids =
  ...
  if (server->backend->resolve_key_bulk(
        server->backend, keys_cnt, (const char *const *)server->keys,
        server->key_ids, server->key_id_lens, &contig_alloc) != 0) {
    tsmq_set_err(server->tsmq, TSMQ_ERR_TIMESERIES, "Bulk key lookup failed");
    goto err;
  }
  ...
  for (i = 0; i < keys_cnt; i++) {
    ...
    if (zmq_send(server->broker_socket, server->key_ids[i],
                 server->key_id_lens[i],
                 ZMQ_SNDMORE) != server->key_id_lens[i]) {
      tsmq_set_err(server->tsmq, TSMQ_ERR_MALLOC,
...
static int recv_key_val(tsmq_server_t *server, zmq_msg_t *key_msg_out,
                        tsmq_val_t *value_out)
{
  ...
  if ((len = zmq_recv(server->broker_socket, value_out, sizeof(tsmq_val_t),
                      0)) != sizeof(tsmq_val_t)) {
    if (len == 0) {
  ...
  *value_out = ntohll(*value_out);
  ...
  if (zmq_msg_init(key_msg_out) == -1) {
    tsmq_set_err(server->tsmq, TSMQ_ERR_MALLOC, "Could not init key message");
    goto err;
  }
...
static int handle_set_bulk(tsmq_server_t *server)
{
  ...
  if (zmq_send(server->broker_socket, "", 0, 0) == -1) {
    tsmq_set_err(server->tsmq, errno, "Could not send set single reply");
    goto err;
  }
  ...
  if (key_cnt > BULK_KEY_THRESHOLD) {
    ...
    if (server->backend->set_bulk_init(server->backend, key_cnt, time) != 0) {
      tsmq_set_err(server->tsmq, TSMQ_ERR_TIMESERIES, "Set bulk init failed\n");
      goto err;
    }
  }
  ...
  while (1) {
    ...
    if (key_cnt <= BULK_KEY_THRESHOLD) {
      if (server->backend->set_single_by_id(
            server->backend, zmq_msg_data(&key_msg), zmq_msg_size(&key_msg),
            value, time) != 0) {
        tsmq_set_err(server->tsmq, TSMQ_ERR_TIMESERIES, "Set single failed\n");
        goto err;
      }
    } else {
      if (server->backend->set_bulk_by_id(server->backend,
                                          zmq_msg_data(&key_msg),
                                          zmq_msg_size(&key_msg), value) != 0) {
        tsmq_set_err(server->tsmq, TSMQ_ERR_TIMESERIES, "Set bulk failed\n");
        goto err;
      }
    }
...
static int handle_request(tsmq_server_t *server)
{
  ...
  if (zmq_send(server->broker_socket, &req_type, tsmq_request_msg_type_size_t,
               ZMQ_SNDMORE) != tsmq_request_msg_type_size_t) {
    tsmq_set_err(server->tsmq, TSMQ_ERR_MALLOC,
  ...
  default:
    tsmq_set_err(server->tsmq, TSMQ_ERR_PROTOCOL, "Unhandled request type (%d)",
                 req_type);
    return -1;
    break;
  }
...
static int run_server(tsmq_server_t *server)
{
  ...
  default:
    ...
    default:
      tsmq_set_err(server->tsmq, TSMQ_ERR_PROTOCOL,
                   "Invalid message type received from broker (%d)", msg_type);
      goto err;
      break;
    }
    break;
