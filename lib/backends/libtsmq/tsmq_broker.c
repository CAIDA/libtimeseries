...
static int client_bind(tsmq_broker_t *broker)
{
  ...
  /* add to reactor */
  if (zloop_reader(broker->loop, broker->client_socket, handle_client_msg,
                   broker) != 0) {
    tsmq_set_err(broker->tsmq, TSMQ_ERR_MALLOC,
                                         ...
                                         zmq_msg_t *identity)
{
  ...
  server->expiry =
    zclock_time() + (broker->heartbeat_interval * broker->heartbeat_liveness);
                                        ...
                                        zmq_msg_t *identity)
{
  ...
  if ((khiter = kh_get(str_server, broker->servers, id)) ==
...
static int server_send_headers(tsmq_broker_t *broker,
                               tsmq_broker_server_t *server,
                               tsmq_msg_type_t msg_type, int sndmore)
{
  zmq_msg_t id_cpy;
  if (server == NULL) {
  ...
  if (zmq_msg_send(&id_cpy, broker->server_socket, ZMQ_SNDMORE) == -1) {
    ...
    tsmq_set_err(broker->tsmq, errno, "Could not send server id to server %s",
                 server->id);
    return -1;
  }
  ...
  if (zmq_send(broker->server_socket, &msg_type, tsmq_msg_type_size_t,
    ...
    tsmq_set_err(broker->tsmq, errno,
                 "Could not send msg type (%d) to server %s", msg_type,
                 server->id);
    return -1;
  }
...
static int handle_server_msg(zloop_t *loop, zsock_t *reader, void *arg)
{
  ...
  if (zmq_msg_init(&server_id) == -1) {
    tsmq_set_err(broker->tsmq, TSMQ_ERR_MALLOC, "Failed to init msg");
    goto err;
  }
  ...
  if ((server = server_get(broker, &server_id)) == NULL) {
    if (msg_type == TSMQ_MSG_TYPE_READY) {
      ...
      tsmq_set_err(broker->tsmq, TSMQ_ERR_PROTOCOL, "Unknown server found");
      goto err;
    }
  }
  ...
  default:
    tsmq_set_err(broker->tsmq, TSMQ_ERR_PROTOCOL,
                 "Invalid message type (%d) rx'd from server", msg_type);
    goto err;
  }
...
static int handle_client_msg(zloop_t *loop, zsock_t *reader, void *arg)
{
  ...
  if (server != NULL &&
      server_send_headers(broker, server, TSMQ_MSG_TYPE_REQUEST, ZMQ_SNDMORE) !=
        0) {
    goto err;
  }
  ...
  while (1) {
    ...
    if (server == NULL) {
      ...
      if (frame_cnt < 3) {
        if (zmq_msg_init(&copy) == -1 || zmq_msg_copy(&copy, &proxy) == -1) {
          zmq_msg_close(&copy);
          goto err;
        }
      ...
      if (zmq_msg_send(&proxy, broker->server_socket, more) == -1) {
        zmq_msg_close(&proxy);
        tsmq_set_err(broker->tsmq, errno, "Could not send message to server");
        goto err;
      }
    }
  ...
  if (zmq_send(broker->client_socket, &ack_type, sizeof(uint8_t), 0) !=
      sizeof(uint8_t)) {
    tsmq_set_err(broker->tsmq, errno, "Could not send request type message");
    return -1;
  }
...
static int init_reactor(tsmq_broker_t *broker)
{
  ...
  if ((broker->timer_id = zloop_timer(broker->loop, broker->heartbeat_interval,
                                      0, handle_heartbeat_timer, broker)) < 0) {
    tsmq_set_err(broker->tsmq, TSMQ_ERR_MALLOC,
