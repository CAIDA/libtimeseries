...
int timeseries_backend_dbats_init(timeseries_backend_t *backend, int argc,
                                  char **argv)
{
  timeseries_backend_dbats_state_t *state;

  /* allocate our state */
  if ((state = malloc_zero(sizeof(timeseries_backend_dbats_state_t))) == NULL) {
  ...
  /* the two parameters that we specify with 0's are only used when creating a
     DB */
  if (dbats_open(&state->dbats_handler, state->dbats_path, 0, 0,
                 state->dbats_flags, 0644) != 0) {
                                          ...
                                          timeseries_kp_t *kp)
{
  ...
  TIMESERIES_KP_FOREACH_KI(kp, ki, id)
  {
    ...
    if (dbats_get_key_id(state->dbats_handler, NULL,
                         timeseries_kp_ki_get_key(ki), dbats_id,
                         DBATS_CREATE) != 0) {
...
retry:
  ...
  TIMESERIES_KP_FOREACH_KI(kp, ki, id)
  {
    ...
    dbats_id = (uint32_t *)timeseries_kp_ki_get_backend_state(
      ki, TIMESERIES_BACKEND_ID_DBATS);
                                           ...
                                           uint32_t key_cnt, uint32_t time)
{
  ...
  /* select the snapshot */
  if (dbats_select_snap(state->dbats_handler, &state->bulk_snap, time, 0) !=
      0) {
    timeseries_log(__func__, "dbats_select_snap failed");
    return -1;
  }
                                            ...
                                            uint8_t **backend_key)
{
  ...
  if (dbats_get_key_id(state->dbats_handler, NULL, key, dbats_id,
                       DBATS_CREATE) != 0) {
    timeseries_log(__func__, "Could not resolve DBATS key ID");
    return -1;
  }
...
int timeseries_backend_dbats_resolve_key_bulk(
  ...
  uint8_t **backend_keys, size_t *backend_key_lens, int *contig_alloc)
{
  ...
  do {
    ...
    rc = dbats_bulk_get_key_id(state->dbats_handler, NULL, &dbats_keys_cnt,
                               keys, dbats_ids, DBATS_CREATE);
    if (rc != 0) {
      retries--;
      if (retries == 0) {
        timeseries_log(__func__,
        ...
        timeseries_log(__func__, "Retrying key lookup for %" PRIu32 " keys",
                       dbats_keys_cnt);
      }
    }
  } while (rc != 0);
