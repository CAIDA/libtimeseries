...
static void usage(const char *name)
{
  fprintf(stderr,
          ...
          "                          (default: %s)\n"
          "       -n <key-cnt>       Number of keys to lookup and insert fake "
          "data for (default: %d)\n"
          ...
          "                            (default: %d)\n",
          name, TSMQ_CLIENT_BROKER_URI_DEFAULT, KEY_LOOKUP_CNT,
...
int main(int argc, char **argv)
{
  ...
  if ((response = tsmq_client_key_lookup(client, key)) == NULL) {
    tsmq_client_perr(client);
    goto err;
  }
  fprintf(stderr, "done\n");
