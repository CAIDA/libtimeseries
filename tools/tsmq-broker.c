...
static void usage(const char *name)
{
  fprintf(
    ...
    "                          (default: %s)\n",
    name, TSMQ_BROKER_CLIENT_URI_DEFAULT, TSMQ_HEARTBEAT_INTERVAL_DEFAULT,
    TSMQ_HEARTBEAT_LIVENESS_DEFAULT, TSMQ_BROKER_SERVER_URI_DEFAULT);
}
...
int main(int argc, char **argv)
{
  ...
  while (prevoptind = optind, (opt = getopt(argc, argv, ":c:i:l:s:v?")) >= 0) {
